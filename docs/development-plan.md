...
- [x] **6. Round scheduling and due effects**
...
- [x] **7. Multi-defender outgoing combat**
...
- [x] **8. Matchup presets and session review**
...
- [x] **9. Measured performance and final verification**
...
- **Milestone 6:** optional seat cadence visits living opponents before advancing the round/clock, with per-seat combat guards on all generation paths. Arcane Denial, goad, Pact and Ring reminders preserve their actual Magic deadlines across reload/undo; users confirm them in the external playtester. Departed-controller triggers are removed while goad retains its duration. Ring taps share one conditional standing upkeep reminder. Build, lint, typecheck, full Node suite and 28 desktop/mobile browser journeys pass.
- **Milestone 7:** attackers select individual defenders; responses and editable damage steps stay grouped by defender. One atomic commit applies losses, poison, commander damage, lifelink and owner-departure cleanup; one Undo restores all of them. An unanswered Fog prevents the entire combat unless actual unpreventable damage is explicitly confirmed. First-strike loss skips only the departed defender's later step, and ending loss protection checks lethal totals even without damage. Build, lint, typecheck, full Node suite and 30 browser journeys pass, including preservation of another defender's edited fields.
- **Milestone 8:** named device-local presets support save/load/delete and validated 16 KiB file import/export, with separate keys for cross-tab safety and no live totals in saved configurations. Loading only changes a new-run draft and disables applying it to live totals. Review is available during play and after game over, showing the actual ending cause, whole-run answer counter and all retained history (up to 40 entries). Build, lint, typecheck, full Node suite and 34 desktop/mobile browser journeys pass.
- **Milestone 9:** added reproducible native timings, gzip budgets, timestamp-classified mobile 4× CPU measurements and retained CI evidence. The blur experiment showed no reliable improvement and was rejected; no speculative cache, delayed autosave or runtime dependency was added. Screenshot review corrected preset contrast and sticky-header text bleed. Final audit also removes draw reminders for departed recipients. Production build, lint, typecheck, all **235 Node tests**, gzip budgets and **35 browser tests** pass; the desktop copy of the mobile-only measurement is intentionally skipped. The aggregate local npm invocation hit a Windows Node/libuv shutdown assertion after building, so every release stage was successfully rerun separately. GitHub release gates for milestones 1–8 were verified green; the final push runs the same gate. Detailed evidence and limitations are in [performance.md](performance.md).
